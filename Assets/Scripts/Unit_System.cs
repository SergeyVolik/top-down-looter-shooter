...
public partial class Unit_System : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        //Movement
        Entities
            .WithAll<Unit_Routed>().ForEach((ref Unit_Component uc, ref DynamicBuffer<Unit_Buffer> ub, ref LocalTransform trans) =>
            {
                if (ub.Length > 0 && uc.routed)
                {
                    uc.waypointDirection = math.normalize(ub[uc.currentBufferIndex].wayPoints - trans.Position);
                    trans.Position += uc.waypointDirection * uc.speed * deltaTime;
                    if (!uc.reached && math.distance(trans.Position, ub[uc.currentBufferIndex].wayPoints) <= uc.minDistanceReached && uc.currentBufferIndex < ub.Length - 1)
                    {
                    ...
                    else if (uc.reached && math.distance(trans.Position, ub[uc.currentBufferIndex].wayPoints) <= uc.minDistanceReached && uc.currentBufferIndex > 0)
                    {
                        uc.currentBufferIndex = uc.currentBufferIndex - 1;
