...
public partial struct UpdateFollowTargetSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (ft, agent, agentMovement, entity) in 
            SystemAPI.Query<RefRW<FollowTargetComponent>, RefRW<Agent>, RefRW<AgentMovement>>()
            .WithEntityAccess())
        {
            if (agentMovement.ValueRO.reached)
            {
...
public partial struct AgentNavigationSystemV2 : ISystem, ISystemStartStop
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (AgentNavigationAspect ana in SystemAPI.Query<AgentNavigationAspect>().WithAll<UpdateNavigationTarget>())
        {
