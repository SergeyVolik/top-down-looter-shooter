        ...
        Dependency = Entities
            .ForEach((ref FirstPersonPlayer player) =>
            {
                if (HasComponent<FirstPersonCharacterInputs>(player.ControlledCharacter) && HasComponent<FirstPersonCharacterComponent>(player.ControlledCharacter))
                {
                    ...
                    var controllerLocalTransform = GetComponent<LocalToWorld>(player.ControlledCharacter);
                    var characterViewLocalTransfrom = GetComponent<LocalToWorld>(character.CharacterViewEntity);
                    quaternion characterRotation = controllerLocalTransform.Rotation;
                    quaternion localCharacterViewRotation = characterViewLocalTransfrom.Rotation;
