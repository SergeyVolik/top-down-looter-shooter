...
public static class FirstPersonCharacterUtilities
{
    ...
    public static void ComputeFinalRotationsFromRotationDelta(
        ...
        ref LocalTransform localCharacterViewRotation)
    {
        // Yaw
        quaternion yawRotation = quaternion.Euler(math.up() * math.radians(yawPitchDeltaDegrees.x));
        characterRotation.Rotation = math.mul(characterRotation.Rotation, yawRotation);
        ...
        localCharacterViewRotation.Rotation = CalculateLocalViewRotation(viewPitchDegrees, viewRollDegrees);
    }
